...
def extract_version_info(title: str) -> VersionInfo:
    ...
    cleaned_title = re.sub(r'\.(flac|mp3|wav|m4a|ogg|aac|ape|wma)$', '', cleaned_title, flags=re.IGNORECASE)

    # Remove website/source tags (e.g., [putaojie.com], [site.com], etc.)
    # Be careful not to remove version markers like [伴奏]
    cleaned_title = re.sub(r'\s*\[.*?\.com.*?\]\s*$', '', cleaned_title, flags=re.IGNORECASE)
    cleaned_title = re.sub(r'\s*\[.*?网.*?\]\s*$', '', cleaned_title, flags=re.IGNORECASE)
    cleaned_title = re.sub(r'\s*\[.*?site.*?\]\s*$', '', cleaned_title, flags=re.IGNORECASE)
    # Only remove trailing brackets if they contain URLs or site names
    cleaned_title = re.sub(r'\s*\[https?://.*?\]\s*$', '', cleaned_title, flags=re.IGNORECASE)
    cleaned_title = re.sub(r'\s*\[www\..*?\]\s*$', '', cleaned_title, flags=re.IGNORECASE)
    ...
    has_instrumental = bool(re.search(
        r'(人声伴奏|vocal\s+accompaniment|伴奏|instrumental|karaoke|off\s*vocal)',
        title_lower
    ))

    has_harmony = bool(re.search(r'和声伴奏|和声|harmony', title_lower))
    ...
    # Support both half-width () and full-width （） parentheses
    has_special_version = bool(re.search(
        r'([（\(]纯享版[）\)]|[（\(]吟唱版[）\)]|[（\(]explicit\s+version[）\)]|[（\(]singing\s+version[）\)]|[（\(]singing\s+ver\.?[）\)]|[（\(]chorus\s+version[）\)]|[（\(]remix[）\)]|[（\(]solo\s+version[）\)])',
        title_lower
    ))
    ...
    base_title = re.sub(r'\s*[（\(]?\s*(和声伴奏|和声|harmony)\s*[）\)]?\s*', ' ', base_title, flags=re.IGNORECASE)
    base_title = re.sub(r'\s*[\[【].*(和声伴奏|和声|harmony).*[\]】]\s*', ' ', base_title, flags=re.IGNORECASE)

    # Remove special version markers (e.g., 吟唱版, 纯享版, Singing Ver, Remix, etc.)
    # Handle both parenthetical and non-parenthetical formats
    base_title = re.sub(
        r'\s*[（\(][^)）]*(?:纯享版|吟唱版|explicit\s+version|singing\s+version|singing\s+ver\.?|chorus\s+version|remix|solo\s+version)[^)）]*[）\)]\s*',
        ' ',
        base_title,
        flags=re.IGNORECASE
    )
