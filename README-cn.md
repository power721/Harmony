...
### 🎧 QQ音乐集成
- **在线音乐浏览** - 浏览QQ音乐排行榜、歌单、专辑、艺术家
- **多维度搜索** - 支持搜索歌曲、专辑、艺术家、歌单
...
- **智能队列管理** - 支持插入队列和添加队列两种操作，可处理当前页或全部页歌曲
- **下载失败处理** - 自动标记下载失败的云盘歌曲，支持重试下载
...
- **统一封面下载** - 统一的封面下载对话框，支持单曲、专辑、艺术家封面
- **策略模式架构** - 使用策略模式处理不同类型的封面搜索
...
### 🎨 现代化界面
- **Spotify 风格设计** - 简约现代的 UI 设计，支持主题色动态提取
...
- **进度控制**：拖动进度条或点击进度条位置
- **音量控制**：拖动音量滑块
- **均衡器（EQ）**：点击音量滑块旁的 EQ 按钮打开均衡器对话框
- **播放模式**：点击播放模式按钮切换

### 播放引擎切换

1. 打开 `设置 -> 播放`
2. 在 `播放引擎` 中选择 `mpv` 或 `Qt Multimedia`
3. 保存设置并重启应用

说明：
- 默认引擎为 `mpv`
- 若系统缺少 `mpv` 运行库，应用会自动回退到 `Qt Multimedia`
- EQ 在 `mpv` 后端下可生效（Qt 后端为 UI 占位）
...
- **GUI 框架**: PySide6 (Qt6)
- **音频引擎**: 可插拔后端（默认 `mpv`，回退 `Qt Multimedia`）
