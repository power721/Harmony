...
class TestMainWindowPlayerProxy:
    ...
    def test_close_event_force_quit_still_runs_shutdown_cleanup(self, qapp):
        """Force-quit paths should still release playback and download resources before quitting."""
        cloud_download_service = SimpleNamespace(cleanup=Mock())
        download_manager = SimpleNamespace(
            cleanup=Mock(),
            download_completed=SimpleNamespace(disconnect=Mock()),
        ...
        qt_app = SimpleNamespace(quit=Mock())
        fake = SimpleNamespace(
            _now_playing_window=None,
            _bootstrap=SimpleNamespace(
                db=SimpleNamespace(close=Mock()),
        ...
        with patch(
            ...
            "ui.windows.main_window.QApplication.instance",
            return_value=qt_app,
        ):
            MainWindow.closeEvent(fake, event)
